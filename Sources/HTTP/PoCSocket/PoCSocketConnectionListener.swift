    ...
    public func write(_ data: Data) {
        ...
        defer {
            ...
            if self.shouldShutdown {
                print("shouldShutdown in write() causing close on socket \(self.socket?.socketfd ?? -1)/\(self.socket?.uuid.uuidString ?? "N/A"), cancelled state is \(self.readerSource?.isCancelled ?? true)")
                self.close()
            }
        }
        do {
